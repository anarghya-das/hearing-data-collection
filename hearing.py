...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    sound_files = exp_config['sound_files']
    baseline_duration = exp_config['baseline_duration']
    stimuli_repeat = exp_config['stimuli_repeat']
    ...
    sound_files = sound_files * stimuli_repeat
    ...
    # create an object to store info about Routine base
    base = data.Routine(
        name='base',
        components=[polygon],
    )
    ...
    continueRoutine = True
    # update component parameters for each repeat
    # store start times for base
    base.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    base.tStart = globalClock.getTime(format='float')
    base.status = STARTED
    thisExp.addData('base.started', base.tStart)
    base.maxDuration = None
    # keep track of which components have finished
    baseComponents = base.components
    for thisComponent in base.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "base" ---
    base.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        if polygon.status == STARTED:
            ...
            if tThisFlipGlobal > polygon.tStartRefresh + baseline_duration-frameTolerance:
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            base.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in base.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in base.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for base
    base.tStop = globalClock.getTime(format='float')
    base.tStopRefresh = tThisFlipGlobal
    thisExp.addData('base.stopped', base.tStop)
    thisExp.nextEntry()
    ...
    for thisTrial in trials:
        ...
        current_sound.setSound(sound_path, secs=stimulus_dration, hamming=True)
        ...
        while continueRoutine:
            ...
            if current_sound.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > current_sound.tStartRefresh + stimulus_dration-frameTolerance or current_sound.isFinished:
                    # keep track of stop time/frame for later
        ...
        marker_outlet.push_sample([f"response-{name}"])
        ...
        while continueRoutine:
            ...
            if text.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > text.tStartRefresh + response_duration-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        while continueRoutine:
            ...
            if cross_4.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'cross_4.started')
                # update status
            ...
            # if cross_4 is stopping this frame...
            if cross_4.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > cross_4.tStartRefresh + rest_duration-frameTolerance:
                    # keep track of stop time/frame for later
                    cross_4.tStop = t  # not accounting for scr refresh
                    cross_4.tStopRefresh = tThisFlipGlobal  # on global time
                    cross_4.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'cross_4.stopped')
                    # update status
                    cross_4.status = FINISHED
